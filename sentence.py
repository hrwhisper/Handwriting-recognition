...
image = readImage('./data/whatareyoudoing.jpg')
...
words = []
sortinter=[i for i in inter]
sortinter.sort()
most = sortinter[int(len(sortinter)*parition)]
maxnum = sortinter[len(sortinter)-1]
start=0
cw = CorrectWord()
for i in xrange(len(inter)):
    if maxnum-most >= max(most,0)*diff and inter[i] >= most:
        words.append(cw.correct(''.join(ans[start:i+1]).lower()))
        start=i+1
   
words.append(cw.correct(''.join(ans[start:]).lower()))
